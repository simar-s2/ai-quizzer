...
export default function QuizTakingMenu({
  ...
  const answeredCount = Object.keys(responses).filter((key) =>
    responses[Number.parseInt(key)]?.trim(),
  ).length;
  ...
  const handleSubmitQuiz = async () => {
    if (
      !confirm(
        "Submit your quiz? You can review your answers and results after submitting.",
      )
    ) {
      return;
    }
        ...
        <CardFooter className="flex flex-col gap-4 pt-6 border-t">
          ...
          <div className="flex flex-wrap justify-center gap-2 py-2">
            {questions.map((_, i) => {
              ...
              const hasAnswer =
                responses[i] !== undefined && responses[i] !== "";
              ...
              return (
                <Button
                  ...
                  className={clsx("min-w-10", {
                    ...
                    "border-primary":
                      hasAnswer && !isCorrect && !isIncorrect && !isActive,
                  })}
                >
                  {i + 1}
                </Button>

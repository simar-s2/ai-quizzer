...
export class MockGenAIService implements IGenAIService {
  ...
  async generateQuiz(
    ...
    const selectedTypes = params.type?.selectedTypes || [
      "mcq",
      "fill",
      "truefalse",
      "shortanswer",
  ...
  async generateQuizStream(
    ...
    const result = await this.generateQuiz(params);
    const json = JSON.stringify(result);
    const encoder = new TextEncoder();

    // Simulate streaming by sending the JSON in small chunks with delays
  ...
  async markAnswers(answers: AnswerToMark[]): Promise<MarkQuizResponse> {
    ...
    const results: MarkingResult[] = answers.map((answer) => {
      if (
        ...
        answer.question_type === "truefalse" ||
      ...
      if (wordCount >= 50) {
        ...
        feedback =
          "Your answer is comprehensive and covers the key points well. You have demonstrated a strong understanding of the topic with relevant details and examples.";
      } else if (wordCount >= 25) {
        marksAwarded = answer.marks_possible * 0.7;
        correctnessStatus = "partial";
        feedback =
          "Your answer addresses the main concepts but could benefit from more detail and depth. Consider expanding on your explanations with specific examples.";
      } else if (wordCount >= 10) {
        marksAwarded = answer.marks_possible * 0.5;
        correctnessStatus = "partial";
        feedback =
          "Your answer touches on some relevant points but needs more development. Try to provide a more thorough explanation that covers all aspects of the question.";
      } else {
        marksAwarded = answer.marks_possible * 0.2;
        correctnessStatus = "partial";
        feedback =
          "Your answer is too brief to fully address the question. Please provide a more detailed response that demonstrates your understanding of the topic.";
      }
    ...
    if (percentage >= 90) {
      overallFeedback =
        "Outstanding work! You demonstrated excellent understanding of the material.";
    } else if (percentage >= 75) {
      overallFeedback =
        "Great job! You showed strong comprehension of the key concepts.";
    } else if (percentage >= 60) {
      overallFeedback =
        "Good effort! You understand several important concepts. Review the feedback to improve.";
    } else if (percentage >= 50) {
      overallFeedback =
        "You have grasped some basic concepts. Focus on the areas highlighted in the feedback.";
    } else {
      overallFeedback =
        "Keep studying! Review the correct answers carefully and try again.";
    }
  ...
  private generateMockQuestions(
    ...
    const questionBank = [
      ...
      },
      {
        ...
        answer:
          "Mitochondria are the powerhouses of the cell, responsible for generating energy through cellular respiration by producing ATP.",
        explanation:
          "Mitochondria convert nutrients into adenosine triphosphate (ATP) through oxidative phosphorylation.",
        marks: 2,
      },
      {
        type: "shortanswer" as const,
      ...
      },
      {
        ...
        answer:
          "Climate change is primarily caused by human activities including burning fossil fuels, deforestation, and industrial processes that release greenhouse gases. Effects include rising global temperatures, melting ice caps, sea level rise, extreme weather events, biodiversity loss, and impacts on agriculture and human health.",
        explanation:
          "Climate change is one of the most pressing environmental issues.",
        marks: 5,
      },
    ];
